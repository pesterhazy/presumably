...
The practics of code tags is widespread, but semantics vary. Every codebase eventually comes to assigns private meanings to various tags, much like Slack workspaces give meanings to various emojis. At Pitch we use TODO for code that _should_ be changed eventually, but not necessarily today. It is considered acceptable, perhaps even desirable to wait to make the improvement suggested. By contrast, a FIXME is mandatory: we consider the code unfit for merge until the problem is addressed. Our linter reflects this judgment: TODOs are given a pass, while FIXMEs fail the build.
